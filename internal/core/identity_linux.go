...
func proxyIdentity(name string) (uint32, uint32, error) {
	if name != "ctlvps-sb" && name != "ctlvps-sp" && !validSnellUser(name) {
		return 0, 0, errors.New("invalid proxy identity")
	}
	identityMu.Lock()
	defer identityMu.Unlock()
	u, e := user.Lookup(name)
	if _, missing := e.(user.UnknownUserError); missing && os.Geteuid() == 0 {
		cmd := exec.Command("/usr/sbin/useradd", "--system", "--user-group", "--no-create-home", "--home-dir", "/nonexistent", "--shell", "/usr/sbin/nologin", "--comment", "VpsCT isolated proxy", name)
	...
	if e != nil {
		return 0, 0, errors.New("无法解析专用 proxy 系统用户")
	}
	...
	if uid < 100 || uid == 65534 || gid == 0 || u.HomeDir != "/nonexistent" || u.Name != "VpsCT isolated proxy" {
		return 0, 0, errors.New("proxy 系统用户冲突，拒绝复用")
	}
	group, e := user.LookupGroupId(u.Gid)
	if e != nil || group.Name != name {
		return 0, 0, errors.New("proxy 必须使用独立系统组")
	}
	ids, e := u.GroupIds()
	if e != nil {
		return 0, 0, e
	...
	for _, id := range ids {
		if id != u.Gid {
			return 0, 0, errors.New("proxy has supplementary groups")
		}
	}
	...
	for _, line := range strings.Split(string(groupFile), "\n") {
		...
		if fields[0] != name || (fields[3] != "" && fields[3] != name) {
			return 0, 0, errors.New("proxy 组包含其他账户")
		}
	}
	...
	for _, line := range strings.Split(string(passwd), "\n") {
		...
		if len(fields) == 7 && fields[3] == u.Gid && fields[0] != name {
			return 0, 0, errors.New("proxy 组被其他账户复用")
		}
		...
		if fields[0] != name || fields[6] != "/usr/sbin/nologin" {
			return 0, 0, errors.New("proxy 身份不独占或允许交互登录")
		}
	}
	if count != 1 {
		return 0, 0, errors.New("proxy 必须是独立本地系统用户")
	}
...
func validSnellUser(name string) bool {
	...
	p, e := strconv.Atoi(strings.TrimPrefix(name, "ctlvps-sn"))
	return e == nil && p > 0 && p <= 65535 && name == "ctlvps-sn"+strconv.Itoa(p)
}
